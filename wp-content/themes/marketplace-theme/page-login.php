...
// 1. Eksekusi pengecekan login sebelum HTML/Header keluar untuk mencegah bug header sent
...
// 2. Proses form login jika dikirim
if (isset($_POST['foodmarket_login_submit'])) {
    
    // VALIDASI KEAMANAN: Cek apakah token nonce ada dan valid
    if ( ! isset($_POST['foodmarket_login_nonce_field']) || ! wp_verify_nonce($_POST['foodmarket_login_nonce_field'], 'foodmarket_login_action') ) {
        $error_message = 'Token keamanan kadaluwarsa. Silakan muat ulang halaman.';
    } else {
        $creds = array(
        ...
        if (is_wp_error($user)) {
            ...
            // Login sukses, cek role untuk rute tujuan akhir
    ...
    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
        ...
        <form method="post" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="log_username" required class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:border-brand focus:ring-1 focus:ring-brand outline-none transition" placeholder="Masukkan username Anda">
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Password</label>
                <input type="password" name="log_password" required class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:border-brand focus:ring-1 focus:ring-brand outline-none transition" placeholder="••••••••">
            </div>
            ...
            <button type="submit" name="foodmarket_login_submit" class="w-full bg-brand hover:bg-brand-dark text-white font-bold py-3.5 rounded-2xl transition text-sm shadow-md shadow-brand/10 mt-2">
                Masuk Sekarang
            </button>
        </form>
